...
export class HollowCuboid extends Action {
 ...
 function cuboid_hollow(w:number, h:number,direction:string):MapWithOffset{
...
function cubeDraw(w:number, h:number,direction:string):MapWithOffset {
 ...
 function pyramidDraw(w:number, h:number,direction:string):MapWithOffset {
    let map = new Array();
    ...
    const step:number = w / h ; //how much to decrease radius by each iteration
...
function square(w:number, map:Array<BlockLocation>, used:Map<string, boolean>, y:number,direction:string) {
...
function plotLine(x0:number, z0:number, x1:number, z1:number, map:Array<BlockLocation>, used:Map<string, boolean>, y:number, direction:string) {
...
function plotLineLow(x0:number, z0:number, x1:number, z1:number, map:Array<BlockLocation>, used:Map<string, boolean>, y:number, direction:string) {
    let dx = x1 - x0;
    let dz = z1 - z0;
    let zi = 1;
    if (dz < 0) {
...
function plotLineHigh(x0:number, z0:number, x1:number, z1:number, map:Array<BlockLocation>, used:Map<string, boolean>, y:number, direction:string) {
    let dx = x1 - x0;
    let dz = z1 - z0;
    let xi = 1;
    if (dx < 0) {
...
function plot(x:number,z:number, map:Array<BlockLocation>, used:Map<string, boolean>, y:number, direction:string) {
